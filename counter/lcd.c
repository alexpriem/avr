...
static void lcd_write(uint8_t chip, uint8_t data, uint8_t rs) 
{
    ...
    uint8_t b_db0, b_db1, b_db2, b_db3, b_rs, b_rw; 
...
{
    volatile uint8_t *pin_db0, *pin_db1, *pin_db2, *pin_db3, *p_rs, *p_rw;
    volatile uint8_t *ddr_db0, *ddr_db1, *ddr_db2, *ddr_db3, *p_enable;
    uint8_t b_db0, b_db1, b_db2, b_db3, b_rs, b_rw, b_enable; 
				...
				uint8_t db0, uint8_t db1, uint8_t db2, uint8_t db3 ) 
{
 ...
 if (db0==P_PORTD) {lcd_p_db0[chip]=&PORTD; lcd_pin_db0[chip]=&PIND; lcd_ddr_db0[chip]=&DDRD; DDRD|=b_db0;}

 db1=db1 & P_PORTMASK;
 if (db1==P_PORTA) {lcd_p_db1[chip]=&PORTA; lcd_pin_db1[chip]=&PINA; lcd_ddr_db1[chip]=&DDRA; DDRA|=b_db1;}
 if (db1==P_PORTB) {lcd_p_db1[chip]=&PORTB; lcd_pin_db1[chip]=&PINB; lcd_ddr_db1[chip]=&DDRB; DDRB|=b_db1;}
 if (db1==P_PORTC) {lcd_p_db1[chip]=&PORTC; lcd_pin_db1[chip]=&PINC; lcd_ddr_db1[chip]=&DDRC; DDRC|=b_db1;}
 if (db1==P_PORTD) {lcd_p_db1[chip]=&PORTD; lcd_pin_db1[chip]=&PIND; lcd_ddr_db1[chip]=&DDRD; DDRD|=b_db1;}

 db2=db2 & P_PORTMASK;
 if (db2==P_PORTA) {lcd_p_db2[chip]=&PORTA; lcd_pin_db2[chip]=&PINA; lcd_ddr_db2[chip]=&DDRA; DDRA|=b_db2;}
 if (db2==P_PORTB) {lcd_p_db2[chip]=&PORTB; lcd_pin_db2[chip]=&PINB; lcd_ddr_db2[chip]=&DDRB; DDRB|=b_db2;}
 if (db2==P_PORTC) {lcd_p_db2[chip]=&PORTC; lcd_pin_db2[chip]=&PINC; lcd_ddr_db2[chip]=&DDRC; DDRC|=b_db2;}
 if (db2==P_PORTD) {lcd_p_db2[chip]=&PORTD; lcd_pin_db2[chip]=&PIND; lcd_ddr_db2[chip]=&DDRD; DDRD|=b_db2;}

 db3=db3 & P_PORTMASK;
 if (db3==P_PORTA) {lcd_p_db3[chip]=&PORTA; lcd_pin_db3[chip]=&PINA; lcd_ddr_db3[chip]=&DDRA; DDRA|=b_db3;}
 if (db3==P_PORTB) {lcd_p_db3[chip]=&PORTB; lcd_pin_db3[chip]=&PINB; lcd_ddr_db3[chip]=&DDRB; DDRB|=b_db3;}
 if (db3==P_PORTC) {lcd_p_db3[chip]=&PORTC; lcd_pin_db3[chip]=&PINC; lcd_ddr_db3[chip]=&DDRC; DDRC|=b_db3;}
 if (db3==P_PORTD) {lcd_p_db3[chip]=&PORTD; lcd_pin_db3[chip]=&PIND; lcd_ddr_db3[chip]=&DDRD; DDRD|=b_db3;}
}
