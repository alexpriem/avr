...
int main(void)
{	
	int8_t vol, bal, changed1, changed2;
	char buf[10];
	...
	*/
	...
	encoder_setup (1, P_PC1, P_PC3);
	

	lcd_printf (0, "volume: %d:%d",vol,bal);	
	 
	uart_printf ("start loop \r\n");
	
	for (;;) {
		changed1=encoder_poll_range (0, &vol, 0,100,1);
		changed2=encoder_poll_range (1, &bal, 0,100,1);
		uart_printf ("%d %d %d %d\r\n",vol, bal, changed1, changed2);
